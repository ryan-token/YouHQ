...
Use raw representable structs instead of enums.
If you add a new case to the enum for a new version of the app, then the moment that new version sets a kind to the new case, it will be sync'd to old devices and fail to decode leaving them in a broken state.
All migrations of your data model must be 100% backwards compatible to work with synchronization

```swift
// WRONG — CloudKit will not handle schema updates well
...
struct Node {
  ...
  enum NodeKind: Int, QueryBindable {
    case folder = 0
    case item = 1
    // some other cases which will be added later
  }
}

// CORRECT — use a raw representable struct instead of an enum
struct Node {
  let id: UUID
  let kind: NodeKind

  struct NoteKind: RawRepresentable, QueryBindable {
    let rawValue: Int
    static let folder = Self(rawValue: 0)
    static let item = Self(rawValue: 1)
    // add more later
  }
}
```
